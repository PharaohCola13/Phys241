...
gamma   = 0.9           # Drive strength (dimensionless)
...
# Time array
tmax = 6                # Duration of simulation in units of the drive period T = 2*pi/w
...
# Plot Stuff
    fig1 = plt.figure(1)
    plt.title("Chapter 12 Question 6 Part A")
...
# Other Plot details (legend, etc)
    ...
    ax1a.yaxis.set_major_locator(MultipleLocator(base=pi/2))
...
# Plot Stuff
    ...
    ax2a.set_title("Chapter 12 Question 6 Part BC")
# Plots solution
    for i in range(0,info[1]):
        ax2a.plot(t,phi[:,i],'-',label="$\phi(0)$={:.0g}$\pi$".format(float(info[2][i])/pi))
# Other plot stuff (legend, etc)
    ...
    ax2a.yaxis.set_major_locator(MultipleLocator(base=pi/2))
